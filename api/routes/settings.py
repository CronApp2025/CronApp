...
def update_settings():
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            try:
                # Actualizamos el usuario con una consulta más detallada para debugging
                query = """
                    UPDATE users 
                    SET 
                        nombre = %s,
                ...
                current_app.logger.info(f"Ejecutando consulta: {query} con parámetros: {params}")
                cursor.execute(query, params)
                
                # Importante: realizar commit antes de fetchone
